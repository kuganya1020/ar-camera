...
const TUT_PAGES = 3;
const TUT_STORAGE_KEY = 'myARCamera_tutorialSeen';
...
function initTutorial() {
  ...
  let seen = localStorage.getItem(TUT_STORAGE_KEY);
  if (!seen) {
    setTimeout(() => showTutorial(), 3200);
  } else {
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      if (tutCurrentPage < TUT_PAGES - 1) {
        goToTutPage(tutCurrentPage + 1, false);
      } else {
        closeTutorial();
      }
    });
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', () => {
      let page = parseInt(dot.getAttribute('data-page'));
      goToTutPage(page, page < tutCurrentPage);
    });
  });
...
function closeTutorial() {
  ...
  localStorage.setItem(TUT_STORAGE_KEY, 'true');
}

function goToTutPage(pageIndex, isBack) {
  let pages = document.querySelectorAll('.tutorial-page');
  let dots  = document.querySelectorAll('.tut-dot');
  let nextBtn = document.getElementById('tut-next-btn');
  ...
  pages.forEach((p, i) => {
    ...
    if (i === pageIndex) {
      ...
      if (isBack) p.classList.add('slide-back');
    }
  });

  dots.forEach((d, i) => d.classList.toggle('active', i === pageIndex));
  tutCurrentPage = pageIndex;

  if (nextBtn) {
    nextBtn.textContent = pageIndex === TUT_PAGES - 1 ? 'はじめる 🎉' : '次へ →';
  }
}